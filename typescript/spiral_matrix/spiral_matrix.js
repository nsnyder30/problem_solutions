...
function spiralMatrixIII(rows, cols, rStart, cStart) {
    var res = [[rStart, cStart]];
    var r = rStart;
    var c = cStart;
    var w = 1;
    while (res.length < rows * cols) {
        var rlow = Math.max(-1, rStart - w);
        var clow = Math.max(-1, cStart - w);
        var rhigh = Math.min(rows, rStart + w);
        var chigh = Math.min(cols, cStart + w);
        if (r == -1) {
            c = chigh;
        }
        while (c < chigh) {
