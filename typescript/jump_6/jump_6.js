...
function maxResult(nums, k) {
    ...
    var res = nums.shift() + nums.pop();
    while (i < nums.length) {
        if (nums[i] >= 0) {
            res += nums[i];
            if (i - j >= k) {
                res += leastNegativeSum(nums.slice(j, i), k);
            }
            j = nums.length;
            i++;
            continue;
    ...
    function leastNegativeSum(negs, w) {
        var l = negs.length;
        var targets = negs.slice(l - w, l);
        targets.sort(function (a, b) { return a > b ? -1 : 1; });
        for (var i_1 = l - w - 1; i_1 >= 0; i_1--) {
            negs[i_1] += targets[0];
            var last = negs.pop();
            var idx = targets.indexOf(last);
            targets.splice(idx, 1);
            binInsert(targets, negs[i_1]);
        }
        return Math.max.apply(Math, negs);
    }
    return res;
}
;
function binInsert(arr, v) {
    var l = 0;
    var r = arr.length - 1;
    var m = 0;
    while (r >= l) {
    ...
    arr.splice(Math.ceil((l + r) / 2), 0, v);
}
...
var f = maxResult;
testSol([[1, -1, -2, 4, -7, 3], 2], f, 7);
testSol([[10, -5, -2, 4, 0, 3], 3], f, 17);
testSol([[1, -5, -20, 4, -1, 3, -6, -3], 2], f, 0);
testSol([[-4], 2], f, -4);
testSol([[5], 1], f, 5);
testSol([[-4, 4], 2], f, 0);
testSol([[-4, -3, -5], 1], f, -12);
testSol([[40, 30, -100, -100, -10, -7, -3, -3], 2], f, -40);
