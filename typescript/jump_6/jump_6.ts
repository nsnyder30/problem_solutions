...
function maxResult(nums: number[], k: number): number {
    ...
    function leastNegativeSum(negs: number[], w: number): number {
        let l = negs.length;
        let targets = negs.slice(l - w, l);
        targets.sort((a, b) => a > b ? -1 : 1);
        for (let i = l - w - 1; i >= 0; i--) {
            negs[i] += targets[0];
            let last = negs.pop();
            let idx = targets.indexOf(last);
            targets.splice(idx, 1);
            binInsert(targets, negs[i]);
        }
...
function binInsert(arr: number[], v: number): void {
    let l = 0;
    let r = arr.length - 1;
    let m = 0;
    while (r >= l) {
